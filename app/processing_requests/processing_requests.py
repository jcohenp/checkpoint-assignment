...
app = Flask(__name__)
sqs = boto3.client('sqs', endpoint_url='http://localhost.localstack.cloud:4566')  # LocalStack SQS endpoint
ssm = boto3.client('ssm', endpoint_url='http://localhost.localstack.cloud:4566')  # LocalStack SSM endpoint
...
def process_request():
    try:
        ...
        logger.error(f'HTTP Method: {request.method}')
